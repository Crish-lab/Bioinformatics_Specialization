...
def middleEdge(text1, text2, l1, l2):
    ...
    for y in range(1, m + 1):
        for x in range(1, l1):
            Graph[0][x][y] = max(Graph[0][x - 1][y - 1] + Grade(text1[x], text2[y]), Graph[0][x - 1][y] + indel,
                                 Graph[0][x][y - 1] + indel)

    for y in range(l2 - 2, m - 1, -1):
        for x in range(l1 - 2, -1, -1):
            Graph[1][x][y] = max(Graph[1][x + 1][y + 1] + Grade(text1[x + 1], text2[y + 1]), Graph[1][x + 1][y] + indel,
                                 Graph[1][x][y + 1] + indel)
    maxVal = 0
    maxIndx = -1
    for i in range(l1):

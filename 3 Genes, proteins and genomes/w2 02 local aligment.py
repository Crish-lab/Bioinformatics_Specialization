...
LOCAL ALIGMENT
'''
#INPUT: dos strings de aminoácidos en su forma alfabética
#OUTPUT: el puntaje/score máximo del alineamiento de los dos strings considerando
#  un indel penalty σ = 5
#Se requiere del documento de texto PAM250 y el dataset. En el dataset ingresar
# dos líneas la primera correspondiente al primer string de aminoácidos y la segunda
# correspondiente al segundo string de aminoácidos
...
def buildLocalAlignmentGraph(text1, text2):
    ...
    for x in range(1, l1):
        for y in range(1, l2):
            Graph[x][y] = max(Graph[x - 1][y] - 5, Graph[x][y - 1] - 5, Graph[x - 1][y - 1] + Grade(text1[x], text2[y]),
                              0)
...
def ImportScoreMatrix():
    dataset = open('PAM250.txt').read().strip().split('\n')

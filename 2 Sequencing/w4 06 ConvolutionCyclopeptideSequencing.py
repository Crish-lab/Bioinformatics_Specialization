...
______________________________________ₓ˚. ୭ ˚○◦˚.˚◦○˚ ୧ .˚ₓ______________________________________________
 En éste código se busca obtener aquellos aminoácidos con la puntuación o
 score más alto.
 INPUT: un número entero "M" el cual equivale a la cantidad de aminoácidos
        que se quieren aislar, un número entero "N" el cual equivale al score
        buscado y el espectro experimental.
 OUTPUT:Una serie numérica correspondiente a los aminoácidos más repetidos
        una vez pasaron por el "filtro de convoluting"
 Ingressar en el documento de texto: en la primera línea M, en la segunda
 línea N y en la tercera línea la serie numéerica del espectro.
'''
...
def ComputeConvolutionOfSpectrum(dataset, n):
    ...
    while (i < 2500):
        if bucket[i][1] < lastCnt:
            break
        if 57 <= bucket[i][0] and bucket[i][0] <= 201: # aumentando en 1 al 200 porque recordar el input 0
...
# Ingresar la direcciòn del documento de texto en ambos 'cool_dataset.txt'
dataset = open(dirname('cool_dataset.txt') + 'cool_dataset.txt').read().strip().split()
